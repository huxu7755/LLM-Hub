...
final class RagServiceManager: ObservableObject {
    ...
    func initialize(modelId: String?) async {
        guard let modelId, !modelId.isEmpty else {
            print("[RAG] initialize — no modelId, cleaning up")
        ...
        if initializedModelName == modelId, await embeddingService.isInitialized {
            print("[RAG] initialize — model \(modelId) already loaded, skipping")
            return
        }

        print("[RAG] initialize — loading model \(modelId)")
        ...
        do {
            print("[RAG] initialize — calling embeddingService.initialize at \(onnxURL.lastPathComponent)")
    ...
    func addDocument(chatId: UUID, text: String, fileName: String) async -> Bool {
        guard isConfigured else {
            print("[RAG] addDocument — not configured, skipping")
            return false
        }
        let idStr = chatId.uuidString
        print("[RAG] addDocument — chatId=\(idStr.prefix(8)) fileName=\(fileName) textLen=\(text.count)")
        ...
        if isReady {
            ...
            print("[RAG] addDocument — embedding skipped (isReady=false)")
        }
        return true
    }
    ...
    func addGlobalMemory(text: String, fileName: String, metadata: String = "pasted") async -> Bool {
        guard isReady else {
            ...
            print("[Memory] addGlobalMemory — BLOCKED (\(reason)). Memory requires an active embedding model.")
            return false
        }
        print("[Memory] addGlobalMemory — fileName=\(fileName) metadata=\(metadata) textLen=\(text.count)")
    ...
    func clearGlobalMemory() async {
        print("[Memory] clearGlobalMemory — clearing all documents")
    ...
    func removeGlobalDocument(docId: String) async {
        print("[Memory] removeGlobalDocument — docId=\(docId)")
        memoryStore.removeDocument(id: docId)
        // Rebuild ragService global memory from remaining documents.
        await ragService.clear(chatId: globalMemoryChatId)
        populatedChatIds.removeAll()
        await restoreGlobalMemory()
        print("[Memory] removeGlobalDocument — rebuilt with \(memoryStore.documents.count) remaining docs")
    }
    ...
    func updateGlobalMemoryDocument(docId: String, newContent: String) async {
        guard var doc = memoryStore.documents.first(where: { $0.id == docId }) else {
            print("[Memory] updateGlobalMemoryDocument — docId=\(docId) not found")
            return
        }
        print("[Memory] updateGlobalMemoryDocument — docId=\(docId) newLen=\(newContent.count)")
    ...
    private func restoreGlobalMemory() async {
        ...
        print("[Memory] restoreGlobalMemory — restoring \(docs.count) documents into RAG")
        for doc in docs {
            await ragService.addRawDocument(chatId: globalMemoryChatId, content: doc.content, fileName: doc.fileName)
        }
        if isReady {
            ...
            print("[Memory] restoreGlobalMemory — chunked but NOT embedded (isReady=false)")
        }
    }
    ...
    private func ensureTokenizerVocab(in modelDir: URL) throws {
        ...
        print("[RAG] initialize — generated vocab.txt from tokenizer.json (\(vocabLines.count) entries)")
    }
}
